...
class homeframe(Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        # Home Screen "Logo"
        logo = Label(root,bg="lightblue",font=("helvetica",64),text="TravelHUB")
        logo.place(relx=0.25,rely=0.15,relheight=0.5,relwidth=0.5)

        # Home Screen Entry
        hp = Entry(root,bg="white",font="helvetica",justify=CENTER)
        hp.insert(END,"Type in your destination city...")
        hp.place(relx = 0.3, rely = 0.5, relwidth = 0.4, relheight = 0.1)

        # Home Screen Button
        take_button = Button(root, bg="#00BAFF", activebackground="#7EDCFF", text= "Take me there!", font="helvetica", fg="white", activeforeground="white",command= lambda: self.get_GeoCode(hp.get()))
        take_button.place(relx = 0.3, rely = 0.63, relwidth = 0.4, relheight = 0.1)


    def get_GeoCode(self,hp):
...
class mainframe():
    ...
    def load(self,lat,lon,city,country):
        ...
        self.return_button = Button(destinationbox, text= "Return", font="helvetica",bg="#7EDCFF",activebackground="lightblue",command= lambda: self.loadback())
